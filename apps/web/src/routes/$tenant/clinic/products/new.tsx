...
export const Route = createFileRoute("/$tenant/clinic/products/new")({
  component: NewProductPage,
})

function NewProductPage() {
  const { tenant } = useParams({ from: "/$tenant/clinic/products/new" })
  const { t } = useTrans()
  ...
  const mutation = useMutation({
    mutationFn: (values: ProductValues) =>
      apiPost(`/${tenant}/clinic/products`, productToPayload(values)),
    onSuccess: () => {
  ...
  useBreadcrumbTail(t("product.add"))
