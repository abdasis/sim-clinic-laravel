...
} from "./membership-tier-form-dialog.tsx"

export function MembershipTierActionsCell({
  tenant,
  tier,
}: {
  tenant: string
  ...
  const remove = useMutation({
    mutationFn: () => apiDelete(`/${tenant}/clinic/membership-tiers/${tier.id}`),
    onSuccess: () => {
      toast.success(t("membership.deleted"))
    ...
    <>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("general.delete")} — {tier.name}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("membership.delete_desc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
